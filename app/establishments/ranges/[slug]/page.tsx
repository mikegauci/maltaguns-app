...
export default async function RangePage({ params }: { params: { slug: string } }) {
  ...
  if (blogPostsError) {
    console.error(`Range blog posts fetch error for ${range.business_name}: ${blogPostsError.message}`);
    ...
    console.log(`Found ${blogPosts?.length || 0} blog posts for ${range.business_name}`);
  }
