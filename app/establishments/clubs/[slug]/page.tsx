...
export default async function ClubPage({
  ...
  // Fetch club's listings - only show active and non-expired listings
  ...
  if (blogPostsError) {
    console.error(
      `Club blog posts fetch error for ${club.business_name}: ${blogPostsError.message}`
    )
    ...
    console.log(
      `Found ${blogPosts?.length || 0} blog posts for ${club.business_name}`
    )
  }
