...
export async function POST(request: Request) {
  try {
    ...
    if (dropFeaturedError) {
      console.error('Error dropping featured_listings constraint:', dropFeaturedError);
      return NextResponse.json(
        { error: "Failed to drop featured_listings constraint", details: dropFeaturedError },
        { status: 500 }
      );
    }
    ...
    if (addFeaturedError) {
      console.error('Error adding new featured_listings constraint:', addFeaturedError);
      return NextResponse.json(
        { error: "Failed to add new featured_listings constraint", details: addFeaturedError },
        { status: 500 }
      );
    }
    ...
    if (dropCreditsError) {
      console.error('Error dropping credits_featured constraint:', dropCreditsError);
      // Continue with the process, as this is not critical
    }

    return NextResponse.json({
      success: true,
      message: "Database updated successfully to allow multiple featured listings"
    });
  } catch (error) {
