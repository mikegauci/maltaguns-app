...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log(
      `[ADMIN-API] Attempting to update expiry for listing: ${listingId}`
    )
    ...
    if (listingError) {
      console.error(
        `[ADMIN-API] Error fetching listing: ${listingError.message}`
      )
    ...
    console.log(
      `[ADMIN-API] Successfully updated expires_at for listing ${listingId}`
    )
