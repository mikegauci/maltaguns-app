...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log(
      '[WEBHOOK-SINGULAR] Webhook signature:',
    ...
    try {
      ...
      console.error(
        `[WEBHOOK-SINGULAR] Signature verification failed: ${err.message}`
      )
    ...
    if (event.type === 'checkout.session.completed') {
      console.log(
        '[WEBHOOK-SINGULAR] Processing checkout.session.completed event'
      )
      ...
      console.log(
        '[WEBHOOK-SINGULAR] Session payment status:',
      ...
      try {
        ...
        if (txCheckError) {
          console.error(
            '[WEBHOOK-SINGULAR] Error checking completed transactions:',
            txCheckError
          )
        } else if (existingCompletedTx && existingCompletedTx.length > 0) {
          console.log(
            '[WEBHOOK-SINGULAR] Payment already processed, skipping to maintain idempotency'
          )
      ...
      if (credits && userId && creditType) {
        console.log('[WEBHOOK-SINGULAR] Processing credit purchase:', {
          userId,
          credits,
          creditType,
        })
        ...
        try {
          ...
          console.log(
            '[WEBHOOK-SINGULAR] Looking for transaction with payment ID:',
            session.id
          )
          ...
          if (findError) {
            console.error(
              '[WEBHOOK-SINGULAR] Error finding transaction:',
              findError
            )
          } else {
            ...
            if (
              ...
              console.log(
                '[WEBHOOK-SINGULAR] Transaction already completed, skipping to prevent duplication'
              )
              return NextResponse.json({
                skipped: true,
            ...
            const { data: recentCredits, error: recentError } =
              await supabaseAdmin
                .from('credit_transactions')
                .select('*')
                .eq('user_id', userId)
            ...
            if (!recentError && recentCredits && recentCredits.length > 0) {
              console.log(
                '[WEBHOOK-SINGULAR] Already processed this payment ID, skipping'
              )
              return NextResponse.json({
                skipped: true,
                reason: 'Payment already processed',
          ...
          console.log(
            '[WEBHOOK-SINGULAR] Updating credit transaction status to completed'
          )
          ...
          if (transactionError) {
            console.error(
              '[WEBHOOK-SINGULAR] Error updating credit transaction:',
              transactionError
            )
            // Continue anyway, not critical
          } else {
            console.log(
              '[WEBHOOK-SINGULAR] Credit transaction updated successfully:',
              txData
            )
          }
          ...
          if (creditType === 'event') {
            ...
            const { data: eventCreditData, error: eventCreditError } =
              await supabaseAdmin
                .from('credits_events')
                .select('*')
                .eq('user_id', userId)
            ...
            if (eventCreditError && eventCreditError.code !== 'PGRST116') {
              // PGRST116 = not found
              console.error(
                '[WEBHOOK-SINGULAR] Error getting event credit record:',
                eventCreditError
              )
              return NextResponse.json(
            ...
            if (eventCreditData) {
              ...
              console.log(
                '[WEBHOOK-SINGULAR] Existing event credits found:',
                eventCreditData
              )
              ...
              console.log(
                `[WEBHOOK-SINGULAR] Updating event credits from ${currentCredits} to ${newCredits}`
              )
              ...
              if (updateError) {
                console.error(
                  '[WEBHOOK-SINGULAR] Error updating event credits:',
                  updateError
                )
                return NextResponse.json(
              ...
              console.log(
                `[WEBHOOK-SINGULAR] Updated event credits. New balance: ${newCredits}`
              )
            } else {
              // Create new event credit record
              console.log(
                '[WEBHOOK-SINGULAR] No existing event credits found, creating new record'
              )
              ...
              if (insertError) {
                console.error(
                  '[WEBHOOK-SINGULAR] Error creating event credit record:',
                  insertError
                )
                return NextResponse.json(
              ...
              console.log(
                `[WEBHOOK-SINGULAR] Created new event credit record with ${credits} credits`
              )
            }
          } else {
            ...
            if (creditError && creditError.code !== 'PGRST116') {
              // PGRST116 = not found
              console.error(
                '[WEBHOOK-SINGULAR] Error getting credit record:',
                creditError
              )
              return NextResponse.json(
            ...
            if (creditData) {
              ...
              console.log(
                '[WEBHOOK-SINGULAR] Existing credits found:',
                creditData
              )
              ...
              console.log(
                `[WEBHOOK-SINGULAR] Updating credits from ${currentCredits} to ${newCredits}`
              )
              ...
              if (updateError) {
                console.error(
                  '[WEBHOOK-SINGULAR] Error updating credits:',
                  updateError
                )
                return NextResponse.json(
              ...
              console.log(
                `[WEBHOOK-SINGULAR] Updated credits. New balance: ${newCredits}`
              )
            } else {
              // Create new credit record
              console.log(
                '[WEBHOOK-SINGULAR] No existing credits found, creating new record'
              )
              ...
              if (insertError) {
                console.error(
                  '[WEBHOOK-SINGULAR] Error creating credit record:',
                  insertError
                )
                return NextResponse.json(
              ...
              console.log(
                `[WEBHOOK-SINGULAR] Created new credit record with ${credits} credits`
              )
            }
          }

          console.log(
            '[WEBHOOK-SINGULAR] Webhook processing completed successfully'
          )
          return NextResponse.json({ success: true, credits_added: credits })
        } catch (error) {
          console.error(
            '[WEBHOOK-SINGULAR] Unexpected error processing credit purchase:',
            error
          )
          return NextResponse.json(
      ...
      if (userId && listingId) {
        console.log(
          '[WEBHOOK-SINGULAR] Processing for userId:',
          userId,
          'listingId:',
          listingId
        )
        ...
        try {
          ...
          if (listingError) {
            console.error(
              '[WEBHOOK-SINGULAR] Error fetching listing details:',
              listingError
            )
            return NextResponse.json(
          ...
          // Update the transaction status to completed
          console.log(
            '[WEBHOOK-SINGULAR] Updating transaction status to completed'
          )
          const { data: txData, error: transactionError } = await supabaseAdmin
            .from('credit_transactions')
            .update({
          ...
          if (transactionError) {
            console.error(
              '[WEBHOOK-SINGULAR] Error updating transaction:',
              transactionError
            )
            // Continue anyway, not critical
          } else {
            console.log(
              '[WEBHOOK-SINGULAR] Transaction updated successfully:',
              txData
            )
          }
          ...
          if (featureError) {
            console.error(
              '[WEBHOOK-SINGULAR] Error checking for existing feature:',
              featureError
            )
            return NextResponse.json(
          ...
          console.log(
            '[WEBHOOK-SINGULAR] Existing feature check result:',
            existingFeature
          )
          ...
          console.log(
            '[WEBHOOK-SINGULAR] New feature period:',
            startDate,
            'to',
            endDateIso
          )
          ...
          console.log(
            '[WEBHOOK-SINGULAR] Days until listing expiry:',
            daysUntilExpiry
          )
          ...
          if (existingFeature && existingFeature.length > 0) {
            ...
            if (updateError) {
              console.error(
                '[WEBHOOK-SINGULAR] Error updating feature:',
                updateError
              )
              return NextResponse.json(
            ...
            // Create new feature
            console.log(
              '[WEBHOOK-SINGULAR] No existing feature found, creating new one'
            )
            ...
            if (insertError) {
              console.error(
                '[WEBHOOK-SINGULAR] Error creating feature:',
                insertError
              )
              return NextResponse.json(
            ...
            console.log(
              `[WEBHOOK-SINGULAR] Created new feature for listing ${listingId}:`,
              insertData
            )
          }
          ...
          if (Object.keys(listingUpdateData).length > 0) {
            console.log(
              '[WEBHOOK-SINGULAR] Updating listing with:',
              listingUpdateData
            )
            ...
            if (listingUpdateError) {
              console.error(
                '[WEBHOOK-SINGULAR] Error updating listing:',
                listingUpdateError
              )
          ...
          console.error(
            '[WEBHOOK-SINGULAR] Unexpected error processing webhook:',
            error
          )
          return NextResponse.json(
