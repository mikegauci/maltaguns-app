...
function CreditsPageComponent() {
  ...
  const fetchData = async () => {
    try {
      ...
      setError(error instanceof Error ? error.message : "Failed to fetch credits data")
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to fetch credits data",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  const refreshData = () => {
    ...
    toast({
      title: "Refreshed",
      description: "Credit data has been refreshed",
    })
  }
  ...
  if (error) {
    return <div className="flex justify-center items-center min-h-screen">
      <div className="text-center p-6 bg-destructive/10 border border-destructive rounded-lg max-w-lg">
        <h2 className="text-xl font-bold text-destructive mb-4">Error Loading Credits</h2>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-muted-foreground">
            {credits.length === 0 ? 'No credits found.' : `Showing ${credits.length} credit records.`}
          </p>
        </div>
